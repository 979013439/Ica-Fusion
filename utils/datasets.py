...
#     sa,sb, t = 'images','labels', []  # /images/, /labels/
#     for x in img_paths:
...
def img2label_paths(img_paths):
    ...
    for img_path in img_paths:
        ...
        # 找到 'visible' 或 'infrared' 目录
        if 'visible' in parts:
            idx = parts.index('visible')
        elif 'infrared' in parts:
            idx = parts.index('infrared')
        else:
            continue
        # 替换为 'labels' 目录
        parts[idx] = 'labels'
        ...
        parts[-1] = os.path.basename(base) + '.txt'
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        n_rgb = len(shapes_rgb)  # number of image
